 ...
 *
 * Issue #944 / #864。外觀對齊「新增單字集」的左右兩欄結構，並遵守一條分工原則：
 * **左側只放設定，右側放所有會被儲存的內容。**
 *
 * - 左側 35% 設定區（`bg-gray-50` 卡）：
 *   1. 最上方兩個 tab —「AI 輔助編輯」（訓練目標／難度／教材描述／產題）
 *      與「上傳圖片 / PDF」（讓 AI 從教材圖片或 PDF 出題）。
 *      與單字集不同，這裡**沒有貼上題目的 textarea** — 最多 10 題，
 *      老師要自己輸入時直接用右側的題目輸入框。
 *   2. 題目計數（MIN_ITEMS ~ MAX_ITEMS，只算有填題目的列）
 *   3. AI 自動翻譯 / AI 生成語音 → 直接沿用 BatchTranslateSettings、
 *      BatchTTSSettings 共用元件；兩者**預設都不勾**，語言也不預選，
 *      避免替老師做他沒做過的決定（與 ReadingAssessmentPanel 一致）。
 *      單字集的「AI 生成例句」在此不適用，故不提供。
 * - 右側（會被儲存的資料）：
 *   1. 「整份設定」可收合卡 — 情境說明（文字＋選填圖片＋語音）／作答指引／
 *      整體評分標準（時間×動貌×語態）。三者皆為所有題目共用。
 *   2. 題目清單 — 標號 + 可拖曳排序 + 可編輯題目/翻譯 + 情境圖片 + 評分準則
 *      （時態/語態沿用整體或本題覆寫、必用字詞逐題獨立、參考答案與補充說明）
 *   右側不自帶卷軸，由外層面板容器統一捲動。
 *
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      setRows((prev) => {
        ...
        return [
          ...kept,
          ...SAMPLE_QUESTIONS.slice(0, Math.max(0, Math.min(5, room))).map(
            (q) => createRow(q),
          ),
        ];
      });
    ...
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-shrink-0 space-y-1.5 pb-4">
        ...
        <input
          ...
          className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500"
        />
      </div>
      ...
      <div className="flex gap-4 flex-1 min-h-0">
                ...
                >
                  {isGenerating ? (
                    ...
                    <>
                      ...
                      {t("scenarioDialogue.buttons.generate")}
                    </>
                  )}
                </Button>
        ...
        {/* ===== 右側：會被儲存的內容（作答指引 + 題目清單） ===== */}
        <div className="flex-1 min-w-0">
          {/* 整份設定：情境說明 / 作答指引 / 整體評分標準，皆為所有題目共用 */}
